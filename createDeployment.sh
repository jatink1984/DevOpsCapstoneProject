#! /bin/bash
CURRENT=$(ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl get service bluegreenlb -o jsonpath='{.spec.selector.app}')
if [ $CURRENT == "blue" ]
then
...
# ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl apply -f blue-green/green/green-deployment.yml
 READY=$(ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl get deploy green -o json | jq '.status.conditions[] | select(.reason == "MinimumReplicasAvailable") | .status' | tr -d '"')
 while [[ "$READY" != "True" ]]; do
   READY=$(ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl get deploy green -o json | jq '.status.conditions[] | select(.reason == "MinimumReplicasAvailable") | .status' | tr -d '"')
   sleep 2
 done
...
#        ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl apply -f blue-green/blue/blue-deployment.yml
        ...
        READY=$(ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl get deploy blue -o json | jq '.status.conditions[] | select(.reason == "MinimumReplicasAvailable") | .status' | tr -d '"')
        while [[ "$READY" != "True" ]]; do
          READY=$(ssh ec2-user@ec2-3-133-144-139.us-east-2.compute.amazonaws.com kubectl get deploy blue -o json | jq '.status.conditions[] | select(.reason == "MinimumReplicasAvailable") | .status' | tr -d '"')
          sleep 2
        done
